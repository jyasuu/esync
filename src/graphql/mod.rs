...
use crate::{config::{Config, EntityConfig, PgType, RelationConfig, RelationKind}, elastic::EsClient};
use anyhow::Result;
...
pub fn build_schema(cfg: &Config, pool: Arc<PgPool>, es: Arc<EsClient>) -> Result<Schema> {
    ...
    let (builder, query) = search::register_search(&cfg, Arc::clone(&pool), Arc::clone(&es), builder, query);
    ...
    for rel in &entity.relations {
        ...
        let return_type = match rel.kind {
            RelationKind::BelongsTo =>
                TypeRef::named(&rel.target),                   // nullable single
            RelationKind::HasMany | RelationKind::ManyToMany =>
                TypeRef::named_nn_list_nn(&rel.target),        // non-null list
        };
        ...
        obj = obj.field(Field::new(field_name, return_type, move |ctx| {
            ...
            FieldFuture::new(async move {
                ...
                let target = cfg.entity(&rel.target)
                    .ok_or_else(|| async_graphql::Error::new(
                        format!("Unknown relation target: {}", rel.target)
                    ))?;

                let cols: Vec<&str> = target.columns.iter()
                    .filter(|c| c.graphql)
                    .map(|c| c.name.as_str())
                    .collect();
                ...
                Ok(match rel.kind {
                    RelationKind::BelongsTo =>
                        items.into_iter().next().map(Some).unwrap_or(None),
                    _ =>
...
async fn fetch_many_to_many(
    ...
    let join_table = rel.join_table.as_deref()
        .ok_or_else(|| anyhow::anyhow!("many_to_many relation '{}' missing join_table", rel.field))?;

    let mut filter_parts = vec![
        format!(
            "{}.{} IN (SELECT {} FROM {join_table} WHERE {} = {})",
            target.table, rel.target_id_col,
            rel.foreign_col, rel.local_col, local_val
        )
    ];
    ...
    Field::new(
        ...
        move |ctx| {
            ...
            FieldFuture::new(async move {
                ...
                let extra_filter = ctx.args.get("filter")
                    .and_then(|v| v.string().ok().map(str::to_owned));
                ...
                Ok(Some(GqlValue::List(rows.into_iter().map(row_to_gql).collect())))
            })
        },
    )
