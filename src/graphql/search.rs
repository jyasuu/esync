...
pub fn register_search(
    ...
    for entity in &cfg.entities {
        if !entity.search.enabled { continue; }
        ...
        let field = build_search_field(entity, Arc::clone(&cfg_arc(cfg)), Arc::clone(&pool), Arc::clone(&es));
        ...
        obj = obj.field(Field::new(
            ...
            move |ctx| {
                ...
                FieldFuture::new(async move {
                    Ok(Some(extract_field(&ctx, &name)))
                })
            },
        ));
    }
        ...
        |ctx| FieldFuture::new(async move {
            let val = match ctx.parent_value.as_value() {
                Some(GqlValue::Object(m)) => {
                    let json_map: serde_json::Map<String, JsValue> = m.iter()
...
fn build_result_type(entity: &EntityConfig) -> Object {
    ...
    // _score
    obj = obj.field(Field::new("_score", TypeRef::named(TypeRef::FLOAT), |ctx| {
        FieldFuture::new(async move { Ok(Some(extract_field(&ctx, "_score"))) })
    }));
    ...
    obj = obj.field(Field::new(
        "_highlight",
        TypeRef::named(&hl_name),
        |ctx| FieldFuture::new(async move {
            Ok(Some(extract_field(&ctx, "_highlight")))
        }),
    ));
    ...
    for rel in &entity.relations {
        let rel_type = match rel.kind {
            ...
            | RelationKind::ManyToMany => TypeRef::named_nn_list_nn(&rel.target),
        };
...
fn build_search_field(
    ...
    Field::new(field_name, TypeRef::named_nn(&page_name), move |ctx| {
        ...
        FieldFuture::new(async move {
            ...
            let q: Option<String> = ctx.args.get("q")
                .and_then(|v| v.string().ok().map(str::to_owned));
            let limit: i64  = ctx.args.get("limit")
                .and_then(|v| v.i64().ok()).unwrap_or(20);
            let offset: i64 = ctx.args.get("offset")
                .and_then(|v| v.i64().ok()).unwrap_or(0);
            let sort_arg: Option<String> = ctx.args.get("sort")
                .and_then(|v| v.string().ok().map(str::to_owned));
            let filter_arg: Option<String> = ctx.args.get("filter")
                .and_then(|v| v.string().ok().map(str::to_owned));

            // ── 2. Build ES query ────────────────────────────────────────
            let es_body = build_es_query(&entity, q.as_deref(), filter_arg.as_deref(), sort_arg.as_deref(), limit, offset);
            ...
            let mut hit_maps: Vec<HashMap<String, serde_json::Value>> = hits.iter()
                .map(|hit| {
                    let mut m: HashMap<String, serde_json::Value> = HashMap::new();
                    // Merge _source fields first
                    if let Some(src) = hit["_source"].as_object() {
                        for (k, v) in src { m.insert(k.clone(), v.clone()); }
                    }
                    // Meta fields
            ...
            let ids: Vec<String> = hit_maps.iter()
            ...
            if !entity.search.live_columns.is_empty() && !ids.is_empty() {
                let live_cols: Vec<&str> = entity.search.live_columns.iter()
                    .map(String::as_str)
                    .collect();
                ...
                let live_rows = crate::db::fetch_by_ids(
                    &pool, &entity.table, &entity.id_column,
                    &fetch_cols, &ids, entity.filter.as_deref(),
                ).await?;
            ...
            for rel_name in &entity.search.enrich {
                ...
                let target_cols: Vec<&str> = target_entity.columns.iter()
                    .filter(|c| c.graphql)
                    .map(|c| c.name.as_str())
                    .collect();
                ...
                match rel.kind {
                    RelationKind::BelongsTo => {
                        ...
                        let fk_vals: Vec<String> = hit_maps.iter()
                            .filter_map(|m| m.get(&rel.local_col)
                                .and_then(|v| v.as_str().map(str::to_owned)))
                            .collect();

                        let fetched = crate::db::fetch_by_ids(
                            &pool, &target_entity.table, &rel.foreign_col,
                            &target_cols, &fk_vals,
                            target_entity.filter.as_deref(),
                        ).await?;

                        for hit in &mut hit_maps {
                            let fk = hit.get(&rel.local_col)
                                .and_then(|v| v.as_str())
                                .unwrap_or_default()
                                .to_owned();
                            let related = fetched.get(&fk)
                                .map(|r| JsValue::Object(r.iter()
                    ...
                    RelationKind::HasMany => {
                        ...
                        let local_vals: Vec<String> = hit_maps.iter()
                            .filter_map(|m| m.get(&rel.local_col)
                                .and_then(|v| v.as_str().map(str::to_owned)))
                            .collect();
                        ...
                        let rows    = fetch_related_rows(
                            &pool, &target_entity, &fetch_cols,
                            &rel.foreign_col, &local_vals,
                            rel.filter.as_deref(), order, rel.limit,
                        ).await?;

                        // Group by FK value
                        let mut groups: HashMap<String, Vec<JsValue>> = HashMap::new();
                        for row in rows {
                            let fk = row.get(&rel.foreign_col)
                                .and_then(|v| v.as_str())
                                .unwrap_or_default()
                                .to_owned();
                            let entry = groups.entry(fk).or_default();
                            entry.push(JsValue::Object(
                                row.into_iter().collect()
                            ));
                        }

                        for hit in &mut hit_maps {
                            let local_val = hit.get(&rel.local_col)
                                .and_then(|v| v.as_str())
                                .unwrap_or_default()
                                .to_owned();
                            let related = groups.get(&local_val)
                                .cloned()
                                .unwrap_or_default();
                    ...
                    RelationKind::ManyToMany => {
                        ...
                        let rows = fetch_m2m_rows(
                            &pool, &target_entity, &target_cols,
                            &join_table, &rel, &local_vals,
                        ).await?;

                        // Each row carries the join local_col so we can group
                        let mut groups: HashMap<String, Vec<JsValue>> = HashMap::new();
                        for row in rows {
                            let local_key = row.get(&rel.local_col)
                                .and_then(|v| v.as_str())
                                .unwrap_or_default()
                                .to_owned();
                            groups.entry(local_key).or_default().push(
                                JsValue::Object(row.into_iter().collect())
                            );
                        }
                        for hit in &mut hit_maps {
                            let local_val = hit.get(&rel.local_col)
            ...
            let items: Vec<GqlValue> = hit_maps.into_iter()
                .map(|m| {
                    GqlValue::Object(
                        m.into_iter()
...
fn build_es_query(
    ...
    let query_clause = if let Some(text) = q.filter(|s| !s.is_empty()) {
        let fields: Vec<JsValue> = entity.search.fields.iter()
    ...
    let filter_clause: Option<JsValue> = filter_str
    ...
    let highlight: Option<JsValue> = if entity.search.highlight.is_empty() {
        ...
        let fields: serde_json::Map<String, JsValue> = entity.search.highlight.iter()
            .map(|f| (f.clone(), json!({ "number_of_fragments": 1, "fragment_size": 150 })))
            .collect();
...
async fn fetch_related_rows(
    ...
    if fk_vals.is_empty() { return Ok(vec![]); }
    use sqlx::Row;

    let col_list = cols.join(", ");
    let vals_list = fk_vals.iter()
        .map(|v| format!("'{}'", v.replace('\'', "''")))
        .collect::<Vec<_>>()
        .join(", ");

    let mut filter = format!("{fk_col} IN ({vals_list})");
    if let Some(f) = extra_filter { filter.push_str(&format!(" AND ({f})")); }
    if let Some(f) = &target.filter { filter.push_str(&format!(" AND ({f})")); }
...
async fn fetch_m2m_rows(
    ...
    if local_vals.is_empty() { return Ok(vec![]); }
    use sqlx::Row;
    ...
    let vals_list = local_vals.iter()
        .map(|v| format!("'{}'", v.replace('\'', "''")))
        .collect::<Vec<_>>()
        .join(", ");

    let extra = rel.filter.as_ref()
        .map(|f| format!(" AND ({f})"))
        .unwrap_or_default();

    let target_extra = target.filter.as_ref()
        .map(|f| format!(" AND ({f})"))
        .unwrap_or_default();
...
fn json_to_gql(v: JsValue) -> GqlValue {
    match v {
        ...
        JsValue::Number(n)   => {
            if let Some(i) = n.as_i64()  { GqlValue::Number(i.into()) }
...
fn gql_to_json(v: GqlValue) -> Option<JsValue> {
    Some(match v {
        ...
        GqlValue::Object(m)  => JsValue::Object(
            m.into_iter().filter_map(|(k, v)| gql_to_json(v).map(|j| (k.to_string(), j))).collect()
        ),
        _ => return None,
    })
}
