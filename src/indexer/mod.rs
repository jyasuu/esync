...
pub async fn rebuild_index(
    ...
    tracing::info!(
        "Indexing {} rows from `{}` → `{}`",
        total, entity.table, entity.index
    );
    ...
    loop {
        let rows = db::fetch_rows(
            pool, &entity.table, &columns,
            entity.filter.as_deref(), batch, offset,
        ).await?;
        ...
        for mut row in rows {
            ...
            if let Some(st_cfg) = &entity.search_text {
                match search_text::build(&row, pool, entity, st_cfg, config).await {
                    ...
                    Err(e) => tracing::warn!(
                        "search_text build failed for {}:{id} — {e}", entity.name
                    ),
                }
            }
