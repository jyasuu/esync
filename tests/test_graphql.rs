...
async fn test_list_pagination_no_duplicates() -> Result<()> {
    ...
    let ids_of = |r: &serde_json::Value| -> Vec<String> {
        r["data"]["list_product"].as_array().unwrap()
            .iter().map(|i| i["id"].as_str().unwrap().to_string()).collect()
    };
...
async fn test_list_search_filters_by_text() -> Result<()> {
    ...
    for item in items {
        assert!(item["name"].as_str().unwrap().to_lowercase().contains("widget"));
    }
    srv.abort(); let _ = srv.await;
    Ok(())
}
...
async fn test_get_product_not_found() -> Result<()> {
    let (srv, _pool) = setup().await?;
    let resp = gql(
        r#"{ get_product(id: "00000000-0000-0000-0000-000000000099") { id } }"#, None,
    ).await?;
    assert_no_gql_errors(&resp);
