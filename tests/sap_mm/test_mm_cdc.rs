    ...
    let handle = tokio::spawn(async move {
        ...
        loop {
            let notification = match listener.recv().await {
                Ok(n) => n,
                Err(e) => {
                    tracing::error!("cdc listener error: {e}");
                    break;
                }
            };
...
async fn test_mm_cdc_material_update_propagates() -> Result<()> {
    ...
    assert!(
        updated,
        "Updated material description should propagate to ES within 6 s"
    );
...
async fn test_mm_cdc_material_delete_removes_from_es() -> Result<()> {
    ...
    assert!(
        removed,
        "Deleted material should disappear from ES within 6 s"
    );
...
async fn test_mm_cdc_material_rapid_updates_settle() -> Result<()> {
    ...
    for i in 1u32..=8 {
        sqlx::query(
            "UPDATE material_master SET gross_weight = $1, updated_at = NOW() WHERE id = $2",
        )
        .bind(i as f64 * 3.0)
    ...
    assert!(
        settled,
        "Final gross_weight (24.0) should settle in ES within 10 s"
    );
...
async fn test_mm_cdc_vendor_update_propagates() -> Result<()> {
    ...
    assert!(
        updated,
        "Vendor city update should propagate to ES within 6 s"
    );
...
async fn test_mm_cdc_vendor_delete_removes_from_es() -> Result<()> {
    ...
    assert!(
        before.is_some(),
        "Pacific vendor must be in ES before delete"
    );
    ...
    assert!(
        removed,
        "Deleted vendor should disappear from ES within 6 s"
    );
