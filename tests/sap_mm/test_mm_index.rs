...
async fn test_mm_index_material_fields() -> Result<()> {
    ...
    let doc = es_get(IDX_MATERIAL, MAT_STEEL)
        .await?
        .expect("MAT-1000 must be indexed");
...
async fn test_mm_index_material_search_text_built() -> Result<()> {
    ...
    let doc = es_get(IDX_MATERIAL, MAT_PUMP)
        .await?
        .expect("MAT-2000 must be indexed");
...
async fn test_mm_index_plant_data_price() -> Result<()> {
    ...
    let steel_record = hits
        .iter()
        .find(|h| h["_source"]["material_id"].as_str() == Some(MAT_STEEL));
    assert!(
        steel_record.is_some(),
        "plant_data for MAT-1000 (EK1 group) must be indexed"
    );
    ...
    assert!(
        (price - 85.0).abs() < 0.01,
        "standard_price should be 85.00, got {price}"
    );
...
async fn test_mm_index_vendor_fields() -> Result<()> {
    ...
    let doc = es_get(IDX_VENDOR, VENDOR_ACME)
        .await?
        .expect("Acme must be indexed");
...
async fn test_mm_index_vendor_search_text_built() -> Result<()> {
    ...
    let doc = es_get(IDX_VENDOR, VENDOR_GLOBAL)
        .await?
        .expect("GlobalParts must be indexed");
...
async fn test_mm_index_material_doc_movement_types() -> Result<()> {
    ...
    assert!(
        types.contains(&"101"),
        "Goods receipt (101) must be indexed"
    );
