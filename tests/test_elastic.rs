...
async fn test_create_and_delete_index() -> Result<()> {
    ...
    assert!(
        !es.index_exists(idx).await?,
        "Index must be gone after delete"
    );
    Ok(())
}
...
async fn test_recreate_index_drops_existing_data() -> Result<()> {
    ...
    es.create_index(idx, basic_body()).await?;
    es.put_document(idx, "doc1", json!({ "hello": "world" }))
        .await?;
...
async fn test_get_index_returns_metadata() -> Result<()> {
    ...
    assert!(
        info.get(idx).is_some(),
        "Response must contain the index name as key"
    );
...
async fn test_bulk_index_25_documents() -> Result<()> {
    ...
    let docs: Vec<(String, serde_json::Value)> = (1..=25)
        .map(|i| {
            (
                format!("doc-{i:03}"),
                json!({ "seq": i, "label": format!("item {i}") }),
            )
        })
        .collect();
...
async fn test_put_and_get_mappings() -> Result<()> {
    ...
    assert!(
        ...
        "score field should appear after put_mapping"
    );
...
async fn test_index_template_lifecycle() -> Result<()> {
    ...
    assert!(
        es.get_template(name).await.is_err(),
        "Template must be gone after delete"
    );
    Ok(())
}
...
async fn test_ilm_policy_lifecycle() -> Result<()> {
    ...
    assert!(
        es.get_policy(name).await.is_err(),
        "Policy must be gone after delete"
    );
    Ok(())
}
