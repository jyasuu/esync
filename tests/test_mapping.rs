...
fn uuid_to_keyword() {
    assert_eq!(
        derive_es_type(&col("id", PgType::Uuid)),
        EsFieldType::Keyword
    );
}
#[test]
fn text_to_text() {
    assert_eq!(derive_es_type(&col("n", PgType::Text)), EsFieldType::Text);
}
#[test]
fn varchar_to_text() {
    assert_eq!(
        derive_es_type(&col("s", PgType::Varchar)),
        EsFieldType::Text
    );
}
#[test]
fn int4_to_integer() {
    assert_eq!(
        derive_es_type(&col("i", PgType::Int4)),
        EsFieldType::Integer
    );
}
#[test]
fn int2_to_integer() {
    assert_eq!(
        derive_es_type(&col("i", PgType::Int2)),
        EsFieldType::Integer
    );
}
#[test]
fn int8_to_long() {
    assert_eq!(derive_es_type(&col("i", PgType::Int8)), EsFieldType::Long);
}
#[test]
fn float4_to_float() {
    assert_eq!(
        derive_es_type(&col("f", PgType::Float4)),
        EsFieldType::Float
    );
}
#[test]
fn float8_to_double() {
    assert_eq!(
        derive_es_type(&col("f", PgType::Float8)),
        EsFieldType::Double
    );
}
#[test]
fn numeric_to_scaled() {
    assert_eq!(
        derive_es_type(&col("p", PgType::Numeric)),
        EsFieldType::ScaledFloat
    );
}
#[test]
fn bool_to_boolean() {
    assert_eq!(
        derive_es_type(&col("b", PgType::Bool)),
        EsFieldType::Boolean
    );
}
#[test]
fn timestamptz_to_date() {
    assert_eq!(
        derive_es_type(&col("t", PgType::Timestamptz)),
        EsFieldType::Date
    );
}
#[test]
fn timestamp_to_date() {
    assert_eq!(
        derive_es_type(&col("t", PgType::Timestamp)),
        EsFieldType::Date
    );
}
#[test]
fn date_to_date() {
    assert_eq!(derive_es_type(&col("d", PgType::Date)), EsFieldType::Date);
}
#[test]
fn jsonb_to_object() {
    assert_eq!(
        derive_es_type(&col("j", PgType::Jsonb)),
        EsFieldType::Object
    );
}
#[test]
fn json_to_object() {
    assert_eq!(derive_es_type(&col("j", PgType::Json)), EsFieldType::Object);
}
...
fn text_field_gets_keyword_subfield() {
    ...
    assert_eq!(
        m["properties"]["name"]["fields"]["keyword"]["type"],
        "keyword"
    );
}
...
fn keyword_field_has_no_subfield() {
    ...
    let no_kw =
        m["properties"]["id"].get("fields").is_none() || m["properties"]["id"]["fields"].is_null();
...
fn date_field_has_format_string() {
    ...
    assert!(
        fmt.contains("strict_date_optional_time"),
        "date format string missing"
    );
}
...
fn indexed_false_excludes_field() {
    let cols = vec![
        col("id", PgType::Uuid),
        col_no_index("secret", PgType::Text),
    ];
    let m = build_mappings(&cols);
    assert!(
        m["properties"].get("id").is_some(),
        "id should be in mappings"
    );
    assert!(
        m["properties"].get("secret").is_none(),
        "non-indexed field must be absent"
    );
}
...
fn multiple_fields_all_appear() {
    ...
    for name in ["id", "name", "price", "ts", "meta"] {
        assert!(
            m["properties"].get(name).is_some(),
            "Field {name} missing from mappings"
        );
    }
}
...
fn index_body_has_settings_and_mappings() {
    let body = build_index_body(&[col("id", PgType::Uuid)], 2, 1);
