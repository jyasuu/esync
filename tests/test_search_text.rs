...
async fn index_entity(pool: &sqlx::PgPool, es: &EsClient, cfg: &Config, name: &str) -> Result<()> {
    let entity = cfg.entities.iter().find(|e| e.name == name)
...
async fn test_search_text_own_columns_present() -> Result<()> {
    ...
    assert!(!src["search_text"].is_null(),
    ...
    assert!(text.contains("Alpha Widget"), "search_text must include product name");
    assert!(text.contains("First widget"), "search_text must include description");
...
async fn test_search_text_null_columns_omitted() -> Result<()> {
    ...
    assert!(!text.is_empty(), "search_text should still have name even if description is null");
...
async fn test_search_text_many_to_many_relation_included() -> Result<()> {
    ...
    assert!(text.contains("Alice"),    "search_text must include customer name");
    assert!(text.contains("vip"),      "search_text must include many_to_many tag label");
    assert!(text.contains("wholesale"),"search_text must include all tag labels");
...
async fn test_search_text_field_is_searchable_in_es() -> Result<()> {
    ...
    let result = es.search("test_products", serde_json::json!({
        "query": { "match": { "search_text": "Widget" } }
    })).await?;

    let count = result["hits"]["total"]["value"].as_i64().unwrap_or(0);
    assert!(count >= 1, "ES match on search_text should find products containing 'Widget'");
...
async fn test_search_text_tag_label_is_searchable() -> Result<()> {
    ...
    let result = es.search("test_customers", serde_json::json!({
        "query": { "match": { "search_text": "vip" } }
    })).await?;

    let count = result["hits"]["total"]["value"].as_i64().unwrap_or(0);
    assert_eq!(count, 1, "Only Alice has the 'vip' tag; search should find exactly 1 customer");
...
async fn test_cdc_rebuilds_search_text_on_update() -> Result<()> {
    ...
    assert!(before_text.contains("Alpha Widget"), "initial search_text contains product name");

    // Start CDC watch
    let watch = spawn_watch_ready(cfg).await;

    // Update the product name in PG
    sqlx::query("UPDATE products SET name = 'Super Alpha Widget XL', updated_at = NOW() WHERE id = $1")
    ...
    assert!(updated, "CDC must rebuild search_text with new product name within 6 s");
    ...
    let handle = tokio::spawn(async move {
        ...
        loop {
            let note = match listener.recv().await {
                Ok(n)  => n,
                Err(e) => { tracing::error!("listener: {e}"); break; }
            };
            ...
            match op.as_str() {
                "INSERT" | "UPDATE" => {
                    ...
                    if entity.search_text.is_some() {
                        if let Ok(Some(text)) = indexer::build_search_text_for_id(&pool, entity, &id, &cfg).await {
                            let field = entity.search_text.as_ref().map(|c| c.field.as_str()).unwrap_or("search_text");
                ...
                "DELETE" => { let _ = es.delete_document(&entity.index, &id).await; }
                _ => {}
            }
        }
    });
